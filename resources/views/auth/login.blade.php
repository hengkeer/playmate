...
<body class="min-h-screen flex">
    ...
    <div class="hidden lg:flex flex-col justify-between w-[42%] p-14" style="border-right:1px solid rgb(var(--border-base));background:rgb(var(--bg-base))">
        ...
        {{-- Big headline --}}
        <div>
            <p class="font-display text-[0.65rem] tracking-[.3em] text-[#F97316] uppercase mb-4">Members Only</p>
            <h2 class="font-display uppercase text-white leading-none" style="font-size:clamp(2.8rem,5vw,4.5rem);font-weight:700">
                Back on<br><span style="color:#F97316">Court.</span>
            </h2>
            <p class="text-white/40 text-sm mt-6 leading-relaxed max-w-xs">
                Sign in to find opponents, join events, and track your game.
            </p>
        </div>
    ...
    <div class="flex-1 flex items-center justify-center p-6 lg:p-16">
        <div class="w-full max-w-sm">
            ...
            <p class="font-display text-[0.65rem] tracking-[.3em] text-[#F97316] uppercase mb-2">Welcome Back</p>
            <h1 class="font-display text-4xl uppercase text-white mb-8" style="font-weight:700">Sign In</h1>
            ...
            <form method="POST" action="{{ route('login.post') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="password" name="password" required
                        class="pm-input" placeholder="••••••••">
                </div>

                <div class="pt-1">
                    <button type="submit" class="pm-btn-primary">Enter the Club →</button>
                </div>
            </form>

            <div class="my-6 flex items-center justify-center gap-4">
                <div class="flex-1" style="height:1px;background:linear-gradient(90deg,transparent,rgb(var(--fg) / .08))"></div>
                <span class="font-display text-[0.65rem] tracking-[.2em] text-white/30 uppercase">or</span>
                <div class="flex-1" style="height:1px;background:linear-gradient(-90deg,transparent,rgb(var(--fg) / .08))"></div>
            </div>

            <a href="{{ route('google.login') }}" class="pm-btn-primary" style="background: rgb(var(--fg) / .06); color: rgb(var(--fg)); border: 1px solid rgb(var(--fg) / .10);">
            ...
            <p class="text-center text-sm text-white/40">
                New to PlayMate?
                <a href="{{ route('register') }}" class="text-[#F97316] font-display text-xs tracking-widest hover:text-white ml-1 transition">
